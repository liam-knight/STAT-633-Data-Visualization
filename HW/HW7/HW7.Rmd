...
Complete the code from class to create a proportion stacked area plot that sums to 1.

```{r, warning=FALSE, message=FALSE}
gdp <-
  read_csv("cpiapp/data/gdp_world.csv", na = c("", "..")) %>%
...
gdp_select %>%
  group_by(Year) %>%
  mutate(percentage = GDP / sum(GDP, na.rm = FALSE)) %>%
  ggplot() +
     geom_area(aes(x = Year, y = percentage, fill = Country))
```

# Question 2

Recreate the app that appears in <https://sta633.shinyapps.io/app_cpi/>.

The app reads the `cpi_world.csv` file and allows users to choose the countries for which the CPI will be shown. In addition, the app allows the user to select the base year for all the indexed series. The default values are the `countries_of_interest` demonstrated in class (and in Question 1), with the default base year of 2010.

**Solution:**

App link: <https://liamknight.shinyapps.io/cpi_app/>
...
cpi <- 
  read_csv("cpiapp/data/cpi_world.csv", na = c("", ".."), skip = 4) %>%
